...
public class ELMM extends AbstractClassifier implements
	...
	public static class Inverse implements Serializable {
		/**
		 * 
		 */
	...
	public static class Selm implements Serializable {
		...
		// é€‰æ‹©é¢„æµ‹çš„å±�æ€§ï¼Œå’Œéš�æœºç§�å­�
		...
		public Selm(int elm_type, int numberofHiddenNeurons,
			...
			NumberofHiddenNeurons = numberofHiddenNeurons;
			func = ActivationFunction;
			NumberofOutputNeurons = 1; // é»˜è®¤ä¸ºä¸€ï¼Œæ•°å€¼å�‹è¾“å‡º
		...
		// --------4
		/**
		 * å¦‚æ�œä¸ºå��è¯�æ€§é¢„æµ‹ï¼Œæ��å�– é¢„æµ‹çš„å��è¯� ä¸ªæ•° ï¼Œä»� 0 å¼€å§‹ ï¼Œæ‰€ä»¥è¦� +1
		 * 
		 * @param traindata
		 * @throws NotConvergedException
		 */
		...
		// by myself
		/**
		 * å°†æ•°æ�®å½’ä¸€åŒ–ä¸ºå�Œç²¾åº¦ äºŒç»´æ•°ç»„
		 * 
		 * @param instances
		 * @param nomalization
		 * @throws NotConvergedException
		...
		public void train(Instances instances, double[][] nomalization)
			 ...
			 */
			TransMatrixAtt(traindata, rows, m_classAtt);// ELM ç®—æ³•
														// å°†ç¬¬ä¸€åˆ—ä½œä¸ºé¢„æµ‹å±�æ€§ï¼Œå°†å�Ÿæ�¥çš„é¢„æµ‹å±�æ€§è½¬è‡³ç¬¬ä¸€åˆ—
			/*
			 * for(int k=0; k<rows; k++){
			 * System.out.println(java.util.Arrays.toString(traindata[k])); }
			 */
		...
		// by my self
		/**
		 * éš�æœºåŒ– çŸ©é˜µ å› ä¸ºæœ‰éš�æœºç§�å­�ï¼Œéš�æœºæ•°å�¯æ�§ï¼Œç®—æ³•ç»“æ�œå�¯é‡�ç�°
		 * 
		 * @param rows
		 * @param columns
		 * @param seed
		...
		private static void train() throws NotConvergedException {
			...
			numTrainData = train_set.numRows();// è®­ç»ƒæ•°æ�®è¡Œæ•°
			NumberofInputNeurons = train_set.numColumns() - 1;// è¾“å…¥å±�æ€§ä¸ºå…¨éƒ¨å±�æ€§å‡�å�»è¾“å‡ºå±�æ€§
			/**
			 * è¿™æ ·éš�æœºå‡ºçš„çŸ©é˜µä¸�å�¯æ�§ï¼Œç»“æ�œä¸�å�¯é‡�ç�° InputWeight = (DenseMatrix)
			...
			transT.transpose(T);// T = transT è½¬ç½®
			transP.transpose(P);// P = transP è½¬ç½®
		...
		private double[] testOut(int rows, int columns) {
			...
			NumberofInputNeurons = columns;// test_set.numColumns();// é—®é¢˜
	...
	/**
	 * å½’ä¸€åŒ–æ•°ç»„ï¼Œæ ‡å‡†åŒ– æ•°å€¼å�‹å±�æ€§ï¼Œå�–æœ€å¤§å€¼ Max å‡�å�»æœ€å°�å€¼ Min ä¸ºé™¤æ•°ï¼Œå�Ÿå€¼ x å‡�æœ€å°�å€¼ Min ä¸ºè¢«é™¤æ•°
	 * æ•°ç»„å�–ä¸¤è¡Œï¼Œç¬¬ä¸€è¡Œä¸ºæœ€å¤§å€¼ï¼Œç¬¬äºŒè¡Œä¸ºæœ€å°�å€¼
	 */
